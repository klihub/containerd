...
func (r *result) setOwner(id, kind, qualif, plugin string) error {
	...
	if owner, ok := r.setters[key]; ok {
		...
		which := ""
		if id == "" {
			which = "new container being created"
		} else {
			which = fmt.Sprintf("existing container %q", id)
		}
		return errors.Errorf("plugins %q and %q both tried to set %s of %s",
			owner, plugin, kind, which)
	}
