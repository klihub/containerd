...
func handleContainerExit(ctx context.Context, e *eventtypes.TaskExit, cntr containerstore.Container, c *criService) error {
	...
	if err != nil {
		...
		if _, err = task.Delete(ctx, WithNRISandboxDelete(cntr.SandboxID), c.nri.WithContainerStop(&cntr), containerd.WithProcessKill); err != nil {
			if !errdefs.IsNotFound(err) {
...
func handleSandboxExit(ctx context.Context, e *eventtypes.TaskExit, sb sandboxstore.Sandbox, c *criService) error {
	...
	if err != nil {
		...
		// TODO(random-liu): [P1] This may block the loop, we may want to spawn a worker
		if _, err = task.Delete(ctx, WithNRISandboxDelete(sb.ID), c.nri.WithPodSandboxStop(&sb), containerd.WithProcessKill); err != nil {
			if !errdefs.IsNotFound(err) {
