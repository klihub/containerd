...
type nriRuntime struct {
	...
	nri  *nri.Adaptation // NRI runtime interface
...
func (n *nriRuntime) WithNRIAdjustments(opts ...nrigen.GeneratorOption) containerd.NewContainerOpts {
	return func(ctx context.Context, _ *containerd.Client, c *containers.Container) error {
		...
		g := nrigen.SpecGenerator(&specgen, opts...)
...
func podSandboxToNRI(pod *sstore.Sandbox) *nri.PodSandbox {
	...
	return &nri.PodSandbox{
		...
		Name:           ctrd.GetName(),
		Uid:            ctrd.GetUID(),
		Namespace:      ctrd.GetNamespace(),
		Annotations:    ctrd.GetAnnotations(),
		Labels:         ctrd.GetLabels(),
		CgroupParent:   ctrd.GetCgroupParent(),
		RuntimeHandler: ctrd.GetRuntimeHandler(),
	}
}
...
func containerToNRI(c interface{}) *nri.Container {
	...
	return &nri.Container{
		Id:           ctrd.GetID(),
		PodSandboxId: ctrd.GetPodSandboxID(),
		Name:         ctrd.GetName(),
		State:        ctrd.GetState(),
		Labels:       ctrd.GetLabels(),
		Annotations:  ctrd.GetAnnotations(),
		Args:         ctrd.GetArgs(),
		Env:          ctrd.GetEnv(),
		Mounts:       ctrd.GetMounts(),
		Hooks:        ctrd.GetHooks(),
		Linux: &nri.LinuxContainer{
			Namespaces:  ctrd.GetLinuxNamespaces(),
			Devices:     ctrd.GetLinuxDevices(),
			Resources:   ctrd.GetLinuxResources(),
			OomScoreAdj: nri.Int(ctrd.GetOOMScoreAdj()),
			CgroupsPath: ctrd.GetCgroupsPath(),
		},
	}
}
