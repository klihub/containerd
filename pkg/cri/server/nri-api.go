...
func (a *nriApi) UpdateContainers(ctx context.Context, updates []*api.ContainerUpdate) ([]*api.ContainerUpdate, error) {
	...
	if len(failed) > 0 {
		return failed, errors.New("CRI container updates failed")
	}

	return nil, nil
}

func (a *nriApi) EvictContainers(ctx context.Context, evict []*api.ContainerEviction) ([]*api.ContainerEviction, error) {
	var failed []*api.ContainerEviction
	...
	if len(failed) > 0 {
		return failed, errors.New("CRI container eviction failed")
	}
...
func (p *criContainer) GetDomain() string {
	return nriDomain
}
