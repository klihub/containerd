...
type Config struct {
	...
	// PluginRoles sets up plugin authentication and roles for authenticated plugins.
	PluginRoles []*nri.Role `toml:"plugin_roles" json:"pluginRoles"`
...
func (c *Config) toOptions() []nri.Option {
	...
	if len(c.PluginRoles) > 0 {
		opts = append(opts, nri.WithAuthConfig(&nri.AuthConfig{Roles: slices.Clone(c.PluginRoles)}))
	}
