...
func (t *domainTable) updateContainers(ctx context.Context, updates []*nri.ContainerUpdate) ([]*nri.ContainerUpdate, error) {
	...
	for _, u := range updates {
		...
		if d == nil {
			...
			log.G(ctx).WithError(err).Errorf("NRI update of container %s failed", u.ContainerId)
			if !u.IgnoreFailure {
		...
		err := d.UpdateContainer(namespaces.WithNamespace(ctx, d.GetName()), u)
		if err != nil {
			log.G(ctx).WithError(err).Errorf("NRI update of container %s failed", u.ContainerId)
			if !u.IgnoreFailure {
				failed = append(failed, u)
...
func (t *domainTable) evictContainers(ctx context.Context, evict []*nri.ContainerEviction) ([]*nri.ContainerEviction, error) {
	...
	for _, e := range evict {
		...
		if d == nil {
			err := errdefs.ErrNotFound
			log.G(ctx).WithError(err).Errorf("NRI eviction of container %s failed", e.ContainerId)
			failed = append(failed, e)
			continue
		}

		err := d.EvictContainer(namespaces.WithNamespace(ctx, d.GetName()), e)
		if err != nil {
			log.G(ctx).WithError(err).Errorf("NRI eviction of container %s failed", e.ContainerId)
			failed = append(failed, e)
		}
